...
export default function ProductDetailPage({
  ...
  const availableVariants = useMemo(() => {
    ...
    return productWithStock.product_attributes.filter((attr: any) => {
      if (selectedVariant?.size && attr.size !== selectedVariant.size)
        return false;
      if (selectedVariant?.color && attr.color !== selectedVariant.color)
        return false;
      return true;
    });
  ...
  const selectedVariantStock = useMemo(() => {
    ...
    const variant = productWithStock.product_attributes.find((attr: any) => {
      const sizeMatch =
        !selectedVariant.size || attr.size === selectedVariant.size;
      const colorMatch =
        !selectedVariant.color || attr.color === selectedVariant.color;
  ...
  useEffect(() => {
    if (
      productWithStock?.product_attributes &&
      productWithStock.product_attributes.length > 0
    ) {
      // Auto-select first available variant if none selected
      if (!selectedVariant) {
        const firstAvailableVariant =
          productWithStock.product_attributes.find(
      ...
      const fetchVendor = async () => {
        ...
        try {
          ...
          const response = await fetch(
            `/api/vendors/${productWithStock.vendor_id}`,
            {
              credentials: "include",
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="flex items-center">
                  ...
                  <span className="text-sm text-gray-600 ml-2">
                    {productWithStock.rating
                      ? productWithStock.rating.toFixed(1)
                      : "0.0"}{" "}
                    ({productWithStock.review_count || 0}{" "}
                    {productWithStock.review_count === 1 ? "review" : "reviews"}
                    )
                  </span>
                </div>
                {vendor?.location && (
                        ...
                        productWithStock.product_attributes?.find(
                          (attr: any) =>
                            ...
                            (!selectedVariant?.color ||
                              attr.color === selectedVariant.color)
                        );
                      const isAvailable =
                        variantWithSize && (variantWithSize.quantity || 0) > 0;
                      ...
                      const variantWithColor =
                        productWithStock.product_attributes?.find(
                          (attr: any) =>
                            attr.color === color &&
                            (!selectedVariant?.size ||
                              attr.size === selectedVariant.size)
                        );
                      const isAvailable =
                        variantWithColor &&
                        (variantWithColor.quantity || 0) > 0;
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (matchingVariant) {
                              setSelectedVariant({
                                size:
                                  selectedVariant?.size || matchingVariant.size,
                                color: color,
                                id: matchingVariant.id,
                                quantity: matchingVariant.quantity,
                              });
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center border border-gray-300 rounded-lg w-32">
                  ...
                  <button
                    onClick={() =>
                      setQuantity(
                        Math.min(selectedVariantStock || 1, quantity + 1)
                      )
                    }
                    disabled={
                  ...
                  <span className="text-sm text-gray-600">
                    {selectedVariantStock
                      ? `${selectedVariantStock} in stock${
                          selectedVariant ? ` for selected variant` : ""
                        }`
                      : "Out of stock"}
                  </span>
                </div>
                ...
                <div className="bg-white rounded-lg p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="flex justify-between py-3 border-b border-gray-100">
                      ...
                      <span className="text-gray-600">
                        {productWithStock.category || "General"}
                      </span>
                    </div>
